...
- AI Moderation: Send player messages to the AI model of your choice to decide if they are allowed. (tokens are not included)

![image_260.png](image_260.png)
- Discord Webhooks: Send webhooks when players violate rules.

![image_257.png](image_257.png)
- Private Messaging: Players can message each other even across servers. It can also be turned off with the /msgtoggle command.
...
- Emojis: Create emojis that players can use in the chat to express themselves. The plugin comes with over 80 by default. No texture pack needed.
...
- Social & Command Spy: Admins can monitor player direct messages and commands executed on the network.

![image_248.png](image_248.png)
- Chat Actions: The player's name can be hovered or clicked, which can be used to display statistics or to run commands.
